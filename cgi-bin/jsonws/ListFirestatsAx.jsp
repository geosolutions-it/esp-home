...
String myQuery1;
if(buffer==null)
	myQuery1 = "SELECT geom FROM protected_sites.wdpa_latest WHERE wdpa_id = "+id + ";";
else
	myQuery1 = "SELECT ST_Buffer(geography(geom),"+buffer+")as geom FROM protected_sites.wdpa_latest WHERE wdpa_id = "+id + ";";
...
var pageom=get_value(myQuery1 ,url1,username1 ,password1 );


String myQuery = "SELECT c3.decade,c3.a, c3.ai, SUM(SUM(c3.a)) OVER";
myQuery += "(ORDER BY c3.decade ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)";
myQuery += "      AS acum";
myQuery += " FROM";
myQuery += " (SELECT c2.decade,SUM(a)as a,SUM(ai)as ai";
myQuery += " FROM";
myQuery += " (SELECT c1.decade,ST_area(geography(geom))/10000 as a,ST_area(geography(geomi))/10000 as ai";
myQuery += " FROM";
myQuery += "(SELECT decade, the_geom as geom, ST_Intersection(pageom,the_geom) as geomi";
myQuery += " FROM fires.mcd45a1 as f";
myQuery += " WHERE ST_Intersects(pageom,f.the_geom) AND decade <= " +decadeend+ " AND decade >= " +decadestart+ " ";
myQuery += " GROUP BY decade,geom";
myQuery += " ORDER BY decade) as c1";
myQuery += " GROUP BY c1.decade,a,c1.geomi) as c2";
myQuery += " GROUP BY c2.decade) as c3";
myQuery += " GROUP BY c3.decade,c3.a,c3.ai";
